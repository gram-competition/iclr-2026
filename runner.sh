...
real_files = [f for f in all_npz if '.distcache.' not in f and '.knncache.' not in f]
...
echo ""
echo "=== Step 1b: Build k-NN cache (skips already-cached files) ==="
python3 -c "
import glob, os, sys, multiprocessing as mp
import numpy as np
sys.path.insert(0, '.')
from models.transolver_residual.features import precompute_knn

all_npz = sorted(glob.glob('gram_data/*.npz'))
real_files = [f for f in all_npz if '.distcache.' not in f and '.knncache.' not in f]
files = [f for f in real_files if not os.path.exists(f.replace('.npz', '.knncache.npz'))]
print(f'  {len(files)} files need knn caching (out of {len(real_files)} total)')

def cache_one(path):
    data = np.load(path)
    knn_idx = precompute_knn(data['pos'])
    out = path.replace('.npz', '.knncache.npz')
    np.savez_compressed(out, knn_idx=knn_idx)
    return path

if files:
    with mp.Pool(min(32, mp.cpu_count())) as pool:
        for i, path in enumerate(pool.imap_unordered(cache_one, files), 1):
            print(f'  knn-cached {i}/{len(files)}: {os.path.basename(path)}', flush=True)
    print('  k-NN cache complete.')
else:
    print('  All files already knn-cached.')
"
