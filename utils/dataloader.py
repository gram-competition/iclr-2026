...
__getitem__ returns:
    inputs  = (velocity_in, pos, idcs_airfoil, t, dist_feats)
    target  = velocity_out
...
class GRaMDataset(Dataset):
    def __init__(self, data_dir: str = DATA_DIR, augment: bool = False):
        ...
        self.files   = [f for f in all_npz if ".distcache." not in f]
    ...
    def __getitem__(self, idx: int):
        ...
        return (velocity_in, pos, idcs_airfoil, t, dist_feat), velocity_out


def collate_fn(batch):
    """
    Custom collate: stacks everything except idcs_airfoil (variable-length) and
    dist_feats (kept as lists so the model can index by sample).
    """
    inputs, targets = zip(*batch)
    vel_in_list, pos_list, idcs_list, t_list, dist_feat_list = zip(*inputs)
    ...
    return (velocity_in, pos, idcs_airfoil, t, dist_feats), velocity_out
...
def make_loaders(
    ...
    base = GRaMDataset(data_dir, augment=False)
    n_total = len(base)
